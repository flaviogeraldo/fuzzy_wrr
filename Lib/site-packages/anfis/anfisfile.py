...
"""
Converted and adapted to python 3 by FlávioRocha 2024
"""

import itertools
import numpy as np
import anfis
from anfis.membership.mfDerivs import partial_dMF
...
class AnfisClass:
    ...
    def __init__(self, X, Y, memFunction):
        ...
        self.memFuncs = self.memClass.MFList  # Aqui estamos usando 'memFuncs' com letra minúscula para a variável
    ...
    def LSE(self, A, B, initialGamma=1000.):
        ...
        for i in range(len(coeffMat[:, 0])):
            a = coeffMat[i, :]
            b = np.array(rhsMat[i])
            S = S - (np.dot(np.dot(np.dot(S, np.transpose(np.matrix(a))), np.matrix(a)), S)) / (1 + np.dot(np.dot(S, a), a))
            x = x + (np.dot(S, np.dot(np.transpose(np.matrix(a)), (np.matrix(b) - np.dot(np.matrix(a), x)))))
        return x
    ...
    def trainHybridJangOffLine(self, epochs=5, tolerance=1e-5, initialGamma=1000, k=0.01):
        ...
        while (epoch < epochs) and (convergence is not True):
            ...
            if len(self.errors) != 0 and self.errors[len(self.errors) - 1] < tolerance:
                convergence = True
            ...
            if len(self.errors) >= 4 and (self.errors[-4] > self.errors[-3] > self.errors[-2] > self.errors[-1]):
                k *= 1.1

            if len(self.errors) >= 5 and self.errors[-1] < self.errors[-2] < self.errors[-3] and self.errors[-5] > self.errors[-4]:
                k *= 0.9
        ...
        for alpha in sorted(ANFISObj.memFuncs[columnX][MF][1].keys()):
            ...
            for rowX in range(len(ANFISObj.X)):
                ...
                for colY in range(ANFISObj.Y.ndim):
                    ...
                    dW_dAlpha = senSit * np.array([np.prod([ANFISObj.memClass.evaluateMF(tmpRow)[c][ANFISObj.rules[r][c]] for c in adjCols]) for r in rulesWithAlpha])
