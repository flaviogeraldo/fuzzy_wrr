...
def Multi_Queue_Tandem_Sim(X1_node1, X2_node1, fuzzy):
    # Função de simulação de duas filas em dois nós (tandem)
    ...
    # Parâmetros iniciais de transmissão e pesos WRR para o nó 1 (fila 1 e fila 2)
    tx1_norm_node1, tx2_norm_node1, tx1_norm_node2, tx2_norm_node2 = 0.5, 0.5, 0.5, 0.5
    ...
    for i in range(1, len(X1_node1)):
        # Nó 1 (Fuzzy Logic para Node 1)
        if fuzzy == 1:
            print(".", end="")
            Qr1_norm_node1 = Qr1_node1[i - 1] / Qsize
            Qr2_norm_node1 = Qr2_node1[i - 1] / Qsize
            new_wrr_weight1_node1, new_wrr_weight2_node1, total_tokens_node1 = FRBS_Multi_Queue_WRR(Qr1_norm_node1, Qr2_norm_node1, wrr_weight1_node1, wrr_weight2_node1)
        ...
        # Nó 2 (Fuzzy Logic aplicado ao Nó 2)
        if fuzzy == 1:
            Qr1_norm_node2 = Qr1_node2[i - 1] / Qsize
            Qr2_norm_node2 = Qr2_node2[i - 1] / Qsize
            new_wrr_weight1_node2, new_wrr_weight2_node2, total_tokens_node2 = FRBS_Multi_Queue_WRR(Qr1_norm_node2, Qr2_norm_node2, wrr_weight1_node2, wrr_weight2_node2)
    ...
    rho2_node2 = np.where(tx2_node2 != 0, (tx2_node2 - Co1_node2) / tx2_node2, 0)
    ...
    if fuzzy == 1:
        return (P1_node1, P2_node1, tx1_node1, tx2_node1, rho1_node1, rho2_node1, BO1_node1, BO2_node1, 
                P1_node2, P2_node2, tx1_node2, tx2_node2, rho1_node2, rho2_node2, BO1_node2, BO2_node2, 
                phi1_node1, phi2_node1, phi1_node2, phi2_node2, wrr1_vector_node1, wrr2_vector_node1,
                wrr1_vector_node2, wrr2_vector_node2)
    else:
        return (P1_node1, P2_node1, tx1_node1, tx2_node1, rho1_node1, rho2_node1, BO1_node1, BO2_node1, 
                P1_node2, P2_node2, tx1_node2, tx2_node2, rho1_node2, rho2_node2, BO1_node2, BO2_node2, 
                phi1_node1, phi2_node1, phi1_node2, phi2_node2)
